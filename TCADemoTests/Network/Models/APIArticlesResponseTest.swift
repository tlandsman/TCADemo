...
struct APIArticlesResponseTest {
    
    @Test
    func test_APIArticlesResponse_DecodedSuccessfully() async throws {
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        let expectedArticles =  try decoder.decode(APIArticlesResponse.self, from: jsonData).results
        #expect(expectedArticles.count == 2)
    }
}
